...
export default function CrimsonParticlesCanvas({ staffBurst, cursorPosition }) {
  ...
  useEffect(() => {
    ...
    /* ── Rising golden embers (reduced from 70 → 45) ─────────── */
    ...
    /* ── Crimson smoke puffs (reduced from 14 → 8) ───────────── */
    S.smoke = Array.from({ length: 8 }, () => ({
      x: Math.random() * W(),
    ...
    /* ── Constellation ambient dots (reduced from 40 → 25) ───── */
    S.constellations = Array.from({ length: 25 }, () => ({
      x: Math.random() * W(),
      y: Math.random() * H() * 0.7,
    ...
    const spawnStar = () => {
      if (S.stars.length >= 3) return; // cap max concurrent
      S.stars.push({
        x: Math.random() * W() * 0.6,
        y: Math.random() * H() * 0.4,
    ...
    /* ── 30fps throttle target (33.3ms per frame) ────────────── */
    ...
    const render = (timestamp) => {
      ...
      S.lastFrame = timestamp - (elapsed % FRAME_INTERVAL);
      ...
      /* ── Smoke (batched — no save/restore needed) ──────────── */
      S.smoke.forEach(p => {
        p.x += p.vx;
        p.y += p.vy;
        if (p.y < h * 0.3) { p.y = h + 40; p.x = Math.random() * w; }
      ...
      /* ── Constellation dots + faint lines (single save/restore) */
      ctx.save();
      ...
      /* ── Rising embers (batched, no per-particle shadow) ───── */
      ctx.save();
      ...
      /* ── Cursor trail (capped at 30 max) ───────────────────── */
      const MAX_TRAIL = 30;
      if (S.trail.length > MAX_TRAIL) S.trail.length = MAX_TRAIL;
      ...
      /* ── Staff burst sparks (capped at 100 max) ────────────── */
      const MAX_SPARKS = 100;
      if (S.sparks.length > MAX_SPARKS) S.sparks.length = MAX_SPARKS;
      ctx.save();
